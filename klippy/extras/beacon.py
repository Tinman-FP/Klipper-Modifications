...
class BeaconProbe:
    def __init__(self, config, sensor_id):
        ...
        self.probe_wrapper = BeaconProbeWrapper(self)
        ...
        if register_as_probe:
            sensor_id.tracker.register_probe_sensor(self)
...
class BeaconEndstopRouter:
    def __init__(self, tracker):
        self.tracker = tracker

    def _get_endstop(self):
        return self.tracker.get_active_probe().mcu_probe

    def get_mcu(self):
        return self._get_endstop().get_mcu()

    def add_stepper(self, stepper):
        for beacon in self.tracker.get_probe_sensors():
            beacon.mcu_probe.add_stepper(stepper)

    def get_steppers(self):
        steppers = []
        seen = set()
        for beacon in self.tracker.get_probe_sensors():
            for stepper in beacon.mcu_probe.get_steppers():
                if stepper not in seen:
                    seen.add(stepper)
    ...
    def query_endstop(self, print_time):
        return self._get_endstop().query_endstop(print_time)

    def get_position_endstop(self):
        return self._get_endstop().get_position_endstop()

    def __getattr__(self, attr):
        return getattr(self._get_endstop(), attr)


class BeaconProbeRouter:
    def __init__(self, tracker):
        self.tracker = tracker
        self._mcu_probe = BeaconEndstopRouter(tracker)

    def _get_probe(self):
        return self.tracker.get_active_probe().probe_wrapper
    ...
    def setup_pin(self, pin_type, pin_params):
        ...
        if pin_params["invert"] or pin_params["pullup"]:
            raise pins.error("Can not pullup/invert probe virtual endstop")
        return self._mcu_probe

    def multi_probe_begin(self):
        return self._get_probe().multi_probe_begin()

    def multi_probe_end(self):
        return self._get_probe().multi_probe_end()

    def get_offsets(self, gcmd=None):
        return self._get_probe().get_offsets(gcmd)

    def get_lift_speed(self, gcmd=None):
        return self._get_probe().get_lift_speed(gcmd)

    def run_probe(self, gcmd, *args, **kwargs):
        return self._get_probe().run_probe(gcmd, *args, **kwargs)

    def get_probe_params(self, gcmd=None):
        return self._get_probe().get_probe_params(gcmd)

    def start_probe_session(self, gcmd):
        return self._get_probe().start_probe_session(gcmd)

    def get_status(self, eventtime):
        active = self.tracker.get_active_sensor_name(require_probe=True)
        return {
            "name": "beacon_router",
            "active_sensor": self.tracker.sensor_label(active),
            "probe_sensors": self.tracker.get_probe_sensor_labels(),
        }

    def __getattr__(self, attr):
        return getattr(self._get_probe(), attr)
...
class BeaconEndstopWrapper:
    ...
    def _handle_home_rails_end(self, homing_state, rails):
        if not self.beacon.id.tracker.is_active_sensor(
            self.beacon.id.name, require_probe=True
        ):
            return
...
class BeaconTracker:
    ...
    def sensor_label(self, name):
        return "default" if name is None else name

    def _normalize_sensor_name(self, name):
        if name is None:
            return None
        name = name.strip().lower()
        if name in ("", "default", "unnamed", "none"):
            return None
        return name

    def get_active_sensor_name(self, require_probe=False):
        sensors = self.probe_sensors if require_probe else self.sensors
        if self.active_sensor in sensors:
    ...
    def is_active_sensor(self, name, require_probe=False):
        return self.get_active_sensor_name(require_probe=require_probe) == name

    def get_probe_sensors(self):
        return list(self.probe_sensors.values())

    def get_probe_sensor_labels(self):
        return [self.sensor_label(name) for name in self.probe_sensors.keys()]
    ...
    def set_active_sensor(self, sensor):
        sensor = self._normalize_sensor_name(sensor)
        if sensor not in self.sensors:
            raise self.printer.command_error(
                "Requested sensor '%s' not found, specify a valid sensor."
                % (self.sensor_label(sensor),)
            )
    ...
    def register_probe_sensor(self, beacon):
        sensor = beacon.id.name
    ...
    def _select_handler_sensor(self, handlers, requested_sensor, explicit):
        if explicit:
            return requested_sensor
        for sensor in (
            self.get_active_sensor_name(),
            None,
            self.get_active_sensor_name(require_probe=True),
        ):
            if sensor in handlers:
                return sensor
        if len(handlers) == 1:
    ...
    def dispatch_gcode(self, handlers, gcmd):
        requested_sensor = gcmd.get("SENSOR", None)
        explicit = requested_sensor is not None
        sensor = self._normalize_sensor_name(requested_sensor)
        handler_sensor = self._select_handler_sensor(handlers, sensor, explicit)
        handler = handlers.get(handler_sensor, None)
        if not handler:
            if explicit and sensor is not None:
                raise gcmd.error(
                    "Requested sensor '%s' not found, specify a valid sensor."
                    % (sensor,)
                )
    ...
    def dispatch_webhook(self, req):
        ...
        requested_sensor = req.get("sensor", None)
        explicit = requested_sensor is not None
        sensor = self._normalize_sensor_name(requested_sensor)
        handler_sensor = self._select_handler_sensor(handlers, sensor, explicit)
        handler = handlers.get(handler_sensor, None)
    ...
    def cmd_BEACON_SELECT(self, gcmd):
        ...
        gcmd.respond_info(
            "Active Beacon sensor set to '%s'" % (self.sensor_label(sensor),)
        )

    cmd_BEACON_LIST_help = "List configured Beacon sensors"

    def cmd_BEACON_LIST(self, gcmd):
        if not self.sensors:
            gcmd.respond_info("No Beacon sensors are configured")
            return
        active_sensor = self.get_active_sensor_name()
        lines = []
        for sensor in self.sensors.keys():
            details = []
            if sensor == active_sensor:
                details.append("active")
            if sensor in self.probe_sensors:
                details.append("probe")
            if sensor is None:
                details.append("unnamed")
            if details:
                lines.append(
                    "%s (%s)" % (self.sensor_label(sensor), ", ".join(details))
                )
            else:
                lines.append(self.sensor_label(sensor))
        gcmd.respond_info("Configured Beacon sensors:\n" + "\n".join(lines))

    cmd_BEACON_STATUS_help = "Show Beacon sensor routing status"

    def cmd_BEACON_STATUS(self, gcmd):
        active_sensor = self.get_active_sensor_name()
        probe_sensor = self.get_active_sensor_name(require_probe=True)
        gcmd.respond_info(
            "Beacon status:\n"
            "  active sensor: %s\n"
            "  active probe sensor: %s\n"
            "  probe-capable sensors: %s"
            % (
                self.sensor_label(active_sensor),
                self.sensor_label(probe_sensor),
                ", ".join(self.get_probe_sensor_labels()) or "none",
            )
        )
